...
from TwitterSearch import TwitterSearchException, TwitterSearchOrder
...
def tweet_streaming(keywords=keyword_combo):
    ...
    tweet_frame = pd.DataFrame(tweet_list, columns=(
        'User_id', 'Date', 'User', 'Favourite_count', 'Retweet_count',
        'Followers_count', 'Friends_count', 'Text', 'Time_zone',
        'Location'))
...
def removeusers(tweet_frame):
    ...
    for i in range(0, tweet_frame.shape[0]):
        if any(word in tweet_frame['User'][i]
               for word in filter_users):
            row.append(i)
        elif filter((lambda x: re.search('game', x)),
                    tweet_frame['User'][i]) == ('game'):
            row.append(i)
        elif filter((lambda x: re.search('snakebite', x)),
                    tweet_frame['User'][i]) == ('snakebite'):
            row.append(i)
        elif any(word in tweet_frame['Text'][i]
                 for word in filter_users):
            row.append(i)
...
def date_timestamp(tweet):
    ...
    tweet['Date'] = tweet.apply(lambda r: parse(r.Date).strftime('%d/%m/%Y'),
                                axis=1)
...
def cleantweet(tweet, lemmatiser, stemmer=None):
    ...
    for data in tweet['cleaned']:
        ...
        str_no_punc = "".join(word for word in str_no_username
                              if word not in exclude)
        tokenised = nltk.word_tokenize(str_no_punc)
        stop_free = [word for word in tokenised
...
if __name__ == "__main__":
    ...
    with open(path, 'w') as f:
        cleaned_tweets.to_csv('snakebite1.csv',
